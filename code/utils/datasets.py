...
class dacase2023_task3:
    ''' DCASE 2023 Task 3 dataset
    ...
    def __init__(self, root_dir, cfg, args):
        ...
        self.dataset_dir['dev']['mic']['STARSS23'] = \
            [self.starss23_dir.joinpath('mic_dev').joinpath('dev-train-sony'),
             self.starss23_dir.joinpath('mic_dev').joinpath('dev-train-tau'),
             self.starss23_dir.joinpath('mic_dev').joinpath('dev-test-sony'),
             self.starss23_dir.joinpath('mic_dev').joinpath('dev-test-tau')]
        ...
        self.dataset_dir['eval'] = {
            'foa':  { 'STARSS23': [self.starss23_dir.joinpath('foa_eval')] }, 
            'mic':  { 'STARSS23': [self.starss23_dir.joinpath('mic_eval')] },
            'meta': { 'STARSS23': [] },
            }

        if not args.dataset == 'STARSS23':
            synth_dataset_list = args.dataset.split(',')
            for _synth_dataset in synth_dataset_list:
                self.dataset_dir['dev']['foa'][_synth_dataset] = [self.synth_dir.joinpath(_synth_dataset).joinpath('foa')]
                self.dataset_dir['dev']['mic'][_synth_dataset] = [self.synth_dir.joinpath(_synth_dataset).joinpath('mic')]
