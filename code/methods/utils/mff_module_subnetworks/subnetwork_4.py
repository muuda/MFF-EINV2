...
class mff_subnetwork_4(nn.Module):
    """
    ...
    def __init__(self, tfcm_layer=6):
        ...
        self.stage3_extraction_64x = nn.Sequential(
            FD(cin=256, cout=512),
            nn.Dropout(p=0.05),
            TFCM(cin=512,tfcm_layer = tfcm_layer),
        )
        ...
        self.stage3_exchange_upsample_64to1 = nn.Sequential(
            nn.Conv2d(512, 64, kernel_size=(1, 1)),
            nn.BatchNorm2d(64),
        )
        self.stage3_exchange_downsample_1to4 = nn.Sequential(
            nn.Conv2d(64, 128,  kernel_size=(1,7), stride=(1,4), padding=(0,2), dilation=1, bias=False),
            nn.BatchNorm2d(128),
        )
        self.stage3_exchange_upsample_16to4 = nn.Sequential(
            nn.Conv2d(256, 128, kernel_size=(1, 1)),
            nn.BatchNorm2d(128),
        )
        self.stage3_exchange_upsample_64to4 = nn.Sequential(
            nn.Conv2d(512, 128, kernel_size=(1, 1)),
            nn.BatchNorm2d(128),
        )
        self.stage3_exchange_downsample_1to16 = nn.Sequential(
            nn.Conv2d(64, 64,  kernel_size=(1,7), stride=(1,4), padding=(0,2), dilation=1, bias=False),
            nn.Conv2d(64, 256,  kernel_size=(1,7), stride=(1,4), padding=(0,2), dilation=1, bias=False),
            nn.BatchNorm2d(256),
        )
        self.stage3_exchange_downsample_4to16 = nn.Sequential(
            nn.Conv2d(128, 256,  kernel_size=(1,7), stride=(1,4), padding=(0,2), dilation=1, bias=False),
            nn.BatchNorm2d(256),
        )
        self.stage3_exchange_upsample_64to16 = nn.Sequential(
            nn.Conv2d(512, 256, kernel_size=(1, 1)),
            nn.BatchNorm2d(256),
        )
        self.stage3_exchange_downsample_1to64 = nn.Sequential(
            nn.Conv2d(64, 64,  kernel_size=(1,7), stride=(1,4), padding=(0,2), dilation=1, bias=False),
            nn.Conv2d(64, 64,  kernel_size=(1,7), stride=(1,4), padding=(0,2), dilation=1, bias=False),
            nn.Conv2d(64, 512,  kernel_size=(1,7), stride=(1,4), padding=(0,2), dilation=1, bias=False),
            nn.BatchNorm2d(512),
        )
        self.stage3_exchange_downsample_4to64 = nn.Sequential(
            nn.Conv2d(128, 128,  kernel_size=(1,7), stride=(1,4), padding=(0,2), dilation=1, bias=False),
            nn.Conv2d(128, 512,  kernel_size=(1,7), stride=(1,4), padding=(0,2), dilation=1, bias=False),
            nn.BatchNorm2d(512),
        )
        self.stage3_exchange_downsample_16to64 = nn.Sequential(
            nn.Conv2d(256, 512,  kernel_size=(1,7), stride=(1,4), padding=(0,2), dilation=1, bias=False),
            nn.BatchNorm2d(512),
        )
        
        # Stage 4 - Feature Extraction
        self.stage4_extraction_1x = nn.Sequential(
            TFCM(cin=64,tfcm_layer = tfcm_layer),
        )
        self.stage4_extraction_4x = nn.Sequential(
            TFCM(cin=128,tfcm_layer = tfcm_layer),
        )
        self.stage4_extraction_16x = nn.Sequential(
            TFCM(cin=256,tfcm_layer = tfcm_layer),
        )
        self.stage4_extraction_64x = nn.Sequential(
            TFCM(cin=512,tfcm_layer = tfcm_layer),
        )
        # Stage 4 - Feature Exchange
        self.stage4_exchange_upsample_4to1 = nn.Sequential(
            nn.Conv2d(128, 64, kernel_size=(1, 1)),
            nn.BatchNorm2d(64),
        )
        self.stage4_exchange_upsample_16to1 = nn.Sequential(
            nn.Conv2d(256, 64, kernel_size=(1, 1)),
            nn.BatchNorm2d(64),
        )
        self.stage4_exchange_upsample_64to1 = nn.Sequential(
            nn.Conv2d(512, 64, kernel_size=(1, 1)),
            nn.BatchNorm2d(64),
        )
    ...
    def forward(self, initial_features: torch.Tensor):
        ...
        features_64x = self.stage3_extraction_64x(features_16x)
        ...
        up64t1 = self.stage3_exchange_upsample_64to1(features_64x)
        updated_features_1x = F.interpolate(up4t1,size=(up4t1.shape[2],up4t1.shape[3]*4),mode="nearest")\
                                + F.interpolate(up16t1,size=(up16t1.shape[2],up16t1.shape[3]*16),mode="nearest")\
                                + F.interpolate(up64t1,size=(up64t1.shape[2],up64t1.shape[3]*64),mode="nearest")\
                                + features_1x
        up16t4 = self.stage3_exchange_upsample_16to4(features_16x)
        up64t4 = self.stage3_exchange_upsample_64to4(features_64x)
        updated_features_4x = F.interpolate(up16t4,size=(up16t4.shape[2],up16t4.shape[3]*4),mode="nearest")\
                                + F.interpolate(up64t4,size=(up16t4.shape[2],up64t4.shape[3]*16),mode="nearest")\
                                + self.stage3_exchange_downsample_1to4(features_1x)\
                                + features_4x
        up64t16 = self.stage3_exchange_upsample_64to16(features_64x)
        updated_features_16x = F.interpolate(up64t16,size=(up64t16.shape[2],up64t4.shape[3]*4),mode="nearest")\
                                + self.stage3_exchange_downsample_1to16(features_1x)\
                                + self.stage3_exchange_downsample_4to16(features_4x)\
                                + features_16x
        updated_features_64x = self.stage3_exchange_downsample_1to64(features_1x)\
                                + self.stage3_exchange_downsample_4to64(features_4x)\
                                + self.stage3_exchange_downsample_16to64(features_16x)\
                                + features_64x
        features_1x = self.activation(updated_features_1x)
        features_4x = self.activation(updated_features_4x)
        features_16x = self.activation(updated_features_16x)
        features_64x = self.activation(updated_features_64x)
        
        # Stage 4
        features_1x = self.stage4_extraction_1x(features_1x)
        features_4x = self.stage4_extraction_4x(features_4x)
        features_16x = self.stage4_extraction_16x(features_16x)
        features_64x = self.stage4_extraction_64x(features_64x)
        
        up4t1 = self.stage4_exchange_upsample_4to1(features_4x) 
        up16t1 = self.stage4_exchange_upsample_16to1(features_16x)
        up64t1 = self.stage4_exchange_upsample_64to1(features_64x)
        updated_features_1x = F.interpolate(up4t1,size=(up4t1.shape[2],up4t1.shape[3]*4),mode="nearest")\
                                + F.interpolate(up16t1,size=(up16t1.shape[2],up16t1.shape[3]*16),mode="nearest")\
                                + F.interpolate(up64t1,size=(up64t1.shape[2],up64t1.shape[3]*64),mode="nearest")\
                                + features_1x
